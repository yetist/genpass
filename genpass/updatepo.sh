...
update_sources() {
	for src in `find . -maxdepth 4 -name "*.go"`;do
		...
		if [ -f ${DOMAIN}_src.pot ];then
			...
			msgcat -s ${DOMAIN}.pot ${DOMAIN}_src.pot -o ${DOMAIN}.pot
...
update_tmpl() {
	...
	for tmpl in $tmpls; do
		...
		if [ -f ${DOMAIN}_tmpl.pot ];then
			...
			msgcat -s ${DOMAIN}.pot ${DOMAIN}_tmpl.pot -o ${DOMAIN}.pot
...
update_po() {
	for l in ${LANGS[@]}; do
		...
		if [ -f locale/$l/LC_MESSAGES/${DOMAIN}.po ]; then
			msgmerge -s -U locale/$l/LC_MESSAGES/${DOMAIN}.po ${DOMAIN}.pot
		else
